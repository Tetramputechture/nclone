#!/usr/bin/env python3
"""
Physics-Accurate Pathfinding Validation Visualization

Creates a comprehensive visualization similar to the reference image but showing
the improved physics-accurate pathfinding system with proper movement types.
"""
...
COLORS = {
    ...
    'switch': '#FFA500',      # Orange switch circle
...
def create_physics_accurate_validation():
    """Create physics-accurate pathfinding validation visualization."""
    print("=" * 80)
    print("🎨 CREATING PHYSICS-ACCURATE PATHFINDING VALIDATION")
    print("=" * 80)
    ...
    try:
        ...
        print(f"🗺️  Level: doortest ({level_data.width}x{level_data.height} tiles)")
        ...
        # Find target node (look for switch entities)
        target_x, target_y = None, None
        for entity in entities:
            if entity.get('type') == EntityType.EXIT_SWITCH:  # Exit switch
                target_x = entity['x']
                target_y = entity['y']
                break
        
        if target_x is None:
            # Fallback: find a node reasonably far from ninja
            for node_idx in range(graph.num_nodes):
                if graph.node_mask[node_idx] == 1 and node_idx != ninja_node:
                    node_x = graph.node_features[node_idx, 0]
                    node_y = graph.node_features[node_idx, 1]
                    dist = ((node_x - ninja_position[0])**2 + (node_y - ninja_position[1])**2)**0.5
                    if dist > 150:  # At least 150px away
        ...
        if result.success and result.path and len(result.path) > 1:
            ...
            # Draw path with numbered nodes (like reference image)
            for i, node_idx in enumerate(result.path):
                node_x = graph.node_features[node_idx, 0]
                node_y = graph.node_features[node_idx, 1]
                
                # Draw path node
                node_circle = Circle((node_x, node_y), 6, 
                                   facecolor=COLORS['path_node'], 
                                   edgecolor='white', 
                                   linewidth=2, zorder=18)
                ax.add_patch(node_circle)
                
                # Add node number
                ax.annotate(str(i+1), (node_x, node_y), 
                           ha='center', va='center', 
                           fontsize=8, fontweight='bold', color='white',
                           zorder=19)
                
                # Draw movement arrows between nodes
                if i < len(result.path) - 1:
                    next_node_idx = result.path[i + 1]
                    next_x = graph.node_features[next_node_idx, 0]
                    next_y = graph.node_features[next_node_idx, 1]
                    ...
                    arrow = FancyArrowPatch(
                        (node_x, node_y), (next_x, next_y),
                        arrowstyle='->', mutation_scale=15,
                        color=color, linewidth=linewidth, alpha=0.8, zorder=17
                    )
                    ax.add_patch(arrow)
        ...
        title = f"Physics-Accurate Pathfinding: {path_info}\n"
        ...
        info_text += f"Target: Display: ({target_x:.0f}, {target_y:.0f})\n"
        ...
        # Create legend (like reference image)
        legend_elements = [
            plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=COLORS['ninja'], 
                       markersize=10, label='Ninja'),
            plt.Line2D([0], [0], marker='s', color='w', markerfacecolor=COLORS['locked_door'], 
                       markersize=8, label='Locked Door Switch'),
            plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=COLORS['exit_switch'], 
                       markersize=8, label='Exit Switch'),
            plt.Line2D([0], [0], marker='s', color='w', markerfacecolor=COLORS['one_way'], 
                       markersize=8, label='One Way Platform'),
            plt.Line2D([0], [0], color=COLORS['walk_edge'], linewidth=3, label='Walk Movement'),
            plt.Line2D([0], [0], color=COLORS['jump_edge'], linewidth=3, label='Jump Movement'),
            plt.Line2D([0], [0], color=COLORS['fall_edge'], linewidth=3, label='Fall Movement'),
            plt.Line2D([0], [0], color=COLORS['functional'], linewidth=3, label='Functional')
        ]
        ...
        # Save visualization
        output_file = '/workspace/nclone/physics_accurate_pathfinding_validation.png'
        plt.tight_layout()
        ...
        print(f"\n📊 PHYSICS-ACCURATE PATHFINDING VALIDATION:")
        print(f"   Graph: {graph.num_nodes} nodes, {graph.num_edges} edges")
...
if __name__ == "__main__":
    create_physics_accurate_validation()