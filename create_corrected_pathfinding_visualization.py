#!/usr/bin/env python3
"""
Create corrected pathfinding visualization with accurate positioning and movement types.

This script addresses all the issues identified:
1. Corrects entity positioning offset (1 tile padding)
2. Accurate tile rendering using actual tile definitions
3. Proper pathfinding to switch (not door)
4. Movement type color coding
5. Accurate entity radii rendering
"""
...
def create_detailed_tile_patches(tile_value, tile_x, tile_y):
    ...
    if tile_value == 0:
        ...
        # 45-degree slopes - create accurate triangular shapes
        ...
        # Slope types - create more accurate slope representations
...
def create_detailed_entity_representation(entity_type, x, y):
    ...
    Args:
        ...
        x, y: Entity position (corrected for padding)
...
def create_corrected_pathfinding_visualization():
    """Create corrected pathfinding visualization with accurate positioning and movement types."""
    print("=" * 80)
    print("🎯 CREATING CORRECTED PATHFINDING VISUALIZATION")
    print("=" * 80)
    ...
    try:
        ...
        # Find leftmost locked door switch (CORRECTED: target the switch, not door)
    ...
    try:
        ...
        print("🏗️ Drawing corrected level geometry...")
        for tile_y in range(level_data.height):
            for tile_x in range(level_data.width):
                ...
                if tile_value != 0:  # Non-empty tile
                    # CORRECTED: Account for 1-tile padding in visualization
                    pixel_x = (tile_x + MAP_PADDING) * TILE_PIXEL_SIZE
                    pixel_y = (tile_y + MAP_PADDING) * TILE_PIXEL_SIZE
        ...
        # Draw detailed entities with corrected positioning
        print("🎯 Drawing corrected entities...")
        for entity in level_data.entities:
            entity_type = entity.get("type", 0)
            entity_x = entity.get("x", 0)
            entity_y = entity.get("y", 0)
            
            # CORRECTED: Account for 1-tile padding in entity positioning
        ...
        # Draw ninja starting position with enhanced visibility (corrected position)
        ...
        # Draw target position with enhanced visibility (corrected position)
        ...
        if len(path_coords) >= 2 and len(path_edge_types) > 0:
            ...
            # Add enhanced waypoint markers with movement type colors
        ...
        ax.set_title(f'Corrected Pathfinding Visualization: Ninja → Leftmost Locked Door Switch\n'
                    f'BFS Path: {len(path)} nodes, Movement Types Color-Coded, Accurate Positioning',
...
Ninja: ({ninja_coords[0]:.0f}, {ninja_coords[1]:.0f}) → Corrected: ({corrected_ninja_x:.0f}, {corrected_ninja_y:.0f})
Target: ({target_x}, {target_y}) → Corrected: ({corrected_target_x}, {corrected_target_y})
Path Length: {len(path)} nodes
Algorithm: BFS (Breadth-First Search)
Graph: {graph.num_nodes} nodes, {graph.num_edges} edges
Movement Types: {movement_text}
Corrections Applied: 1-tile padding offset, accurate entity radii
Direct Distance: {((corrected_target_x - corrected_ninja_x)**2 + (corrected_target_y - corrected_ninja_y)**2)**0.5:.1f}px"""
        ...
        output_path = "/workspace/nclone/corrected_pathfinding_visualization.png"
        plt.savefig(output_path, dpi=300, bbox_inches='tight', facecolor='white')
        print(f"✅ Saved corrected pathfinding visualization to: {output_path}")
...
def main():
    """Main function."""
    success = create_corrected_pathfinding_visualization()
    
    if success:
        print("\n" + "=" * 80)
        print("🎉 SUCCESS! Corrected pathfinding visualization created successfully!")
        print("✅ Fixed entity positioning offset (1-tile padding)")
        ...
        print("✅ Visualization saved as corrected_pathfinding_visualization.png")
        print("=" * 80)
        return 0
    else:
        print("\n" + "=" * 80)
        print("❌ FAILED! Could not create corrected pathfinding visualization")
        print("=" * 80)
        return 1
