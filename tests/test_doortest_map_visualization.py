...
def test_doortest_map_issues():
    ...
    print(
        f"✅ Loaded doortest map: ({level_data.width}, {level_data.height}) tiles, {len(level_data.entities)} entities"
    )
    ...
    print(
        f"✅ Built graph with {graph_data.sub_cell_graph.num_nodes} nodes and {graph_data.sub_cell_graph.num_edges} edges"
    )
    ...
    if functional_edges > 0:
        ...
        for i in range(graph_data.sub_cell_graph.num_edges):
            if edge_types[i] == EdgeType.FUNCTIONAL.value:
                ...
                node1_pos = (
                    float(node_features[node1_id, 0]),
                    float(node_features[node1_id, 1]),
                )
                node2_pos = (
                    float(node_features[node2_id, 0]),
                    float(node_features[node2_id, 1]),
                )

                print(
                    f"  Functional edge {functional_count + 1}: {node1_pos} -> {node2_pos}"
                )
                functional_count += 1
    ...
    for i in range(graph_data.sub_cell_graph.num_edges):
        if edge_types[i] == EdgeType.WALK.value:
            ...
            for pos in [node1_pos, node2_pos]:
                ...
                if (
                    0 <= tile_y < level_data.height
                    and 0 <= tile_x < level_data.width
                    and level_data.get_tile(tile_y, tile_x) == 1
                ):
    ...
    if walkable_edges_in_solid == 0:
        print(
            "✅ ISSUE #2 RESOLVED: Walkable edges in solid tiles have been filtered out"
        )
    else:
        ...
        for i, violation in enumerate(solid_tile_violations[:5]):
            print(
                f"  Violation {i + 1}: Edge {violation['edge_id']} at tile {violation['tile_pos']} (value={violation['tile_value']})"
            )
            print(
                f"    Node positions: {violation['node1_pos']} -> {violation['node2_pos']}"
            )
    ...
    start_node = navigation_engine._find_node_at_position(
        graph_data.sub_cell_graph, start_pos
    )
    end_node = navigation_engine._find_node_at_position(
        graph_data.sub_cell_graph, end_pos
    )
    ...
    if start_node is not None and end_node is not None:
        # Test A* navigation
        path_result = navigation_engine.find_shortest_path(
            graph_data.sub_cell_graph, start_node, end_node
        )
        ...
        if path_result.success:
            ...
            if len(path_result.path) > 0:
                ...
                for i, node_id in enumerate(path_result.path[:5]):  # Show first 5 nodes
                    node_pos = (
                        float(node_features[node_id, 0]),
                        float(node_features[node_id, 1]),
                    )
    ...
    for i, test_pos in enumerate(test_positions):
        test_node = navigation_engine._find_node_at_position(
            graph_data.sub_cell_graph, test_pos
        )
        if start_node is not None and test_node is not None:
            path_result = navigation_engine.find_shortest_path(
                graph_data.sub_cell_graph, start_node, test_node
            )
            if path_result.success:
                successful_paths += 1
                print(
                    f"  Test {i + 1}: ✅ Path found to {test_pos} ({len(path_result.path)} nodes, cost {path_result.total_cost:.2f})"
                )
            else:
    ...
    print(
        f"{'✅' if issue1_resolved else '❌'} RESOLVED: Issue #1: Functional edges between switches and doors"
    )
    print(
        f"{'✅' if issue2_resolved else '❌'} RESOLVED: Issue #2: Walkable edges in solid tiles"
    )
    print(
        f"{'✅' if issue3_resolved else '❌'} RESOLVED: Issue #3: Pathfinding not working on traversable paths"
    )
