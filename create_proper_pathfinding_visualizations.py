...
class PathfindingVisualizer:
    ...
    def load_test_map(self, map_path: str):
        ...
        for i, entity in enumerate(level_data.entities):
            ...
            if 0 <= tile_y < level_data.height and 0 <= tile_x < level_data.width:
                ...
                print(
                    f"Entity {i}: type={entity_type}, pos=({entity_x}, {entity_y}), tile=({tile_x}, {tile_y}), tile_type={tile_type}"
                )

                if tile_type != 0:
                    print(
                        f"⚠️  WARNING: Entity {i} is NOT in an empty tile (tile_type={tile_type})"
                    )
                else:
    ...
    def create_visualization(self, map_path: str, output_path: str):
        ...
        for entity in level_data.entities:
            ...
            corrected_x = entity_x - (TILE_PIXEL_SIZE * 1.5)  # Move left by 24px
            corrected_y = entity_y - (TILE_PIXEL_SIZE * 1.5)  # Move down by 24px

            # Convert to canvas coordinates directly
            adj_x = (
                corrected_x / TILE_PIXEL_SIZE + padding
            ) * self.tile_size + self.tile_size / 2
            adj_y = (
                corrected_y / TILE_PIXEL_SIZE + padding
            ) * self.tile_size + self.tile_size / 2
        ...
        try:
            ...
            switch_node = (
                find_closest_node(switch_entity_pos, graph_data)
                if switch_entity_pos
                else None
            )
            ...
            for path_name, result in paths_to_draw:
                ...
                for edge_type in result.edge_types:
                    ...
                    movement_summary[movement_name] = (
                        movement_summary.get(movement_name, 0) + 1
                    )
