...
def validate_observation_ranges(obs):
    ...
    for key in ["player_frame", "global_view"]:
        ...
        print(
            f"  {status} {key:25s} [{arr.min():3d}, {arr.max():3d}]  {nonzero_pct:5.1f}% non-zero"
        )
    ...
    if has_doors:
        ...
        for i in range(5):
            ...
            if np.any(door_data > 0):
                print(
                    f"    door_{i}: switch=({door_data[0]:.3f},{door_data[1]:.3f}) "
...
def validate_temporal_consistency(obs_history):
    ...
    print(
        f"  Ninja position: {steps_with_movement}/{len(pos_changes)} steps with movement"
    )
    ...
    print(
        f"  Reachability: {steps_with_reach_change}/{len(reach_changes)} steps with change"
    )
...
def main():
    ...
    parser.add_argument(
        "--episodes", type=int, default=3, help="Number of episodes to test"
    )
    parser.add_argument(
        "--max-steps", type=int, default=100, help="Max steps per episode"
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Print detailed observation values"
    )
