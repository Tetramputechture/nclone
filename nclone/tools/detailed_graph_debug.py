...
def analyze_graph_building(map_id: int, verbose: bool = True):
    ...
    if adjacency:
        ...
        for (src_type, dst_type), count in sorted(
            tile_pair_counts.items(), key=lambda x: -x[1]
        )[:20]:
        ...
        if problematic:
            ...
            for i, edge in enumerate(problematic[:20]):  # Show first 20
                print(f"\n  #{i + 1}:")
                print(
                    f"    From: {edge['src']} (tile {edge['src_tile']}, type {edge['src_type']})"
                )
                print(
                    f"    To: {edge['dst']} (tile {edge['dst_tile']}, type {edge['dst_type']})"
                )
        ...
        for edge in cross_tile_edges:
            ...
            if src_type == 1 or dst_type == 1:
                solid_edges.append({**edge, "src_type": src_type, "dst_type": dst_type})

        if solid_edges:
            print(f"\n⚠ Found {len(solid_edges)} edges involving solid tiles (Type 1):")
            for i, edge in enumerate(solid_edges[:10]):
                print(
                    f"  #{i + 1}: {edge['src']} (type {edge['src_type']}) → {edge['dst']} (type {edge['dst_type']})"
                )
        else:
