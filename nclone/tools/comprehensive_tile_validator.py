...
4. Compares graph_builder vs tile_connectivity_precomputer implementations
...
def analyze_tile_implementation(tile_type: int) -> Dict:
    ...
    Returns dict with:
    - sub_node_validity_fast: list of 4 bools from graph_builder
...
def render_tile_comparison(tile_type: int, scale: int = 20) -> Image.Image:
    ...
    Shows:
    - Left: graph_builder implementation
    ...
    try:
        font = ImageFont.truetype(
            "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", int(scale * 0.7)
        )
        font_small = ImageFont.truetype(
            "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", int(scale * 0.5)
        )
    except:
    ...
    draw.text(
        (size * scale * 1.5 + scale * 2, subheader_y),
        "tile_connectivity_precomputer",
        fill="blue",
        font=font_small,
        anchor="mm",
    )
    ...
    for impl_idx, check_func in enumerate([fast_graph_check, precomputer_check]):
        ...
        for (offset_x, offset_y), (sub_x, sub_y) in zip(
            ...
            label = f"({sub_x},{sub_y})"
            draw.text(
                (node_x, node_y), label, fill="white", font=font_small, anchor="mm"
            )
...
def create_comprehensive_grid(
    output_path: str = "debug_output/tile_validation_grid.png",
):
    ...
    for tile_type in range(38):
        ...
        if not analysis["consistency"]:
            ...
            print(
                f"⚠️  Type {tile_type:2d} ({TILE_NAMES.get(tile_type, 'Unknown')}): INCONSISTENT"
            )
            print(f"    Fast sub-nodes:     {analysis['sub_node_validity_fast']}")
            print(f"    Precomp sub-nodes:  {analysis['sub_node_validity_precomp']}")
            print(
                f"    Solid pixels:       Fast={analysis['solid_pixel_count_fast']}, Precomp={analysis['solid_pixel_count_precomp']}"
            )
            print()
    ...
    print("  Rendered all 38 tile types" + " " * 20)
...
def create_individual_tiles(output_dir: str = "debug_output/tiles"):
    ...
    for tile_type in problem_types:
        ...
        print(
            f"  Type {tile_type:2d} ({TILE_NAMES.get(tile_type, 'Unknown')}): {status}"
        )
...
def main():
    ...
    print()
    print(
        "This tool compares the two implementations of _check_subnode_validity_simple()"
    )
    ...
    if inconsistencies:
        print(
            f"❌ Found {len(inconsistencies)} tile types with inconsistent implementations:"
        )
