...
def render_tile_ascii(
    tile_type: int, width: int = 24, height: int = 24, show_legend: bool = True
) -> str:
    """
    ...
    for (offset_x, offset_y), (sub_x, sub_y) in zip(SUB_NODE_OFFSETS, SUB_NODE_COORDS):
        ...
        if 0 <= offset_y < height and 0 <= offset_x < width:
            ...
            sub_node_markers.append(
                f"  {node_num}: ({sub_x},{sub_y})@({offset_x},{offset_y}) = {marker}"
            )
    ...
    if show_legend:
        lines.append("")
        lines.append(
            "Legend: █=solid  ·=empty  1-4=valid sub-nodes  X=invalid sub-node"
        )
...
def main():
    ...
    parser.add_argument(
        "--pair",
        nargs=3,
        metavar=("TILE_A", "TILE_B", "DIR"),
        help="Test tile pair traversability",
    )
    parser.add_argument(
        "--matrix", type=int, help="Show connectivity matrix for tile type"
    )
