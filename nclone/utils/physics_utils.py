...
from ..constants.physics_constants import (
    ...
    BOUNCE_BLOCK_BOOST_NEUTRAL,
    BOUNCE_BLOCK_INTERACTION_RADIUS,
    BOUNCE_BLOCK_SIZE,
    GRAVITY_FALL,
    MIN_HORIZONTAL_VELOCITY,
    DEFAULT_MINIMUM_TIME,
    BOUNCE_BLOCK_MAX_COMPRESSION,
    BOUNCE_BLOCK_SPRING_CONSTANT,
    BOUNCE_BLOCK_MIN_COMPRESSION,
    BOUNCE_BLOCK_COMPRESSION_THRESHOLD,
    MAX_JUMP_DISTANCE,
    MAX_FALL_DISTANCE,
    TRAJECTORY_POINT_INTERVAL,
)
...
def calculate_bounce_block_boost_multiplier(
    bounce_blocks: List[Dict[str, Any]], ninja_pos: Tuple[float, float]
) -> float:
    """
    ...
    for block in bounce_blocks:
        ...
        if distance <= BOUNCE_BLOCK_INTERACTION_RADIUS:
            ...
            if state == BounceBlockState.EXTENDING:
                boost = (
                    BOUNCE_BLOCK_BOOST_MIN
                    + (BOUNCE_BLOCK_BOOST_MAX - BOUNCE_BLOCK_BOOST_MIN) * compression
                )
            elif state == BounceBlockState.COMPRESSED:
                boost = (
                    BOUNCE_BLOCK_BOOST_MIN
                    + (BOUNCE_BLOCK_BOOST_MAX - BOUNCE_BLOCK_BOOST_MIN)
                    * compression
                    * 0.8
                )
...
def calculate_compression_amount(
    current_pos: Tuple[float, float], original_pos: Tuple[float, float]
) -> float:
    """
...
def determine_bounce_block_state(
    compression_amount: float, previous_compression: float, ninja_contact: bool
) -> BounceBlockState:
    """
...
def calculate_clearance_directions(
    block_pos: Tuple[float, float], level_entities: List[Dict[str, Any]]
) -> Dict[str, float]:
    """
    ...
    for entity in level_entities:
        ...
        entity_x = (
            entity.get("x", 0.0)
            if isinstance(entity, dict)
            else getattr(entity, "x", 0.0)
        )
        entity_y = (
            entity.get("y", 0.0)
            if isinstance(entity, dict)
            else getattr(entity, "y", 0.0)
        )
...
def calculate_enhanced_bounce_boost(
        ...
        if distance <= BOUNCE_BLOCK_INTERACTION_RADIUS:
            ...
            if state == BounceBlockState.EXTENDING:
                ...
                if (
                    ...
                    and abs(ninja_velocity[1]) > MIN_HORIZONTAL_VELOCITY
                ):  # Ninja is moving
    ...
    if boost_info["repeated_boost_available"] and extending_blocks:
        ...
        if previous_frame_data and previous_frame_data.get(
            "repeated_boost_active", False
        ):
...
def calculate_bounce_block_clearance_boost(
    ...
    for direction, clearance in clearance_data.items():
        if (
            clearance > max_clearance and clearance >= BOUNCE_BLOCK_SIZE
        ):  # At least 1 tile clearance
    ...
    clearance_factor = min(
        1.0, max_clearance / (BOUNCE_BLOCK_SIZE * 3)
    )  # Normalize to 3 tiles max
    boost_multiplier = (
        BOUNCE_BLOCK_BOOST_MIN
        + (BOUNCE_BLOCK_BOOST_MAX - BOUNCE_BLOCK_BOOST_MIN) * clearance_factor
    )

    # Boost direction is opposite to clearance direction
    boost_direction_map = {"up": "down", "down": "up", "left": "right", "right": "left"}
