...
class GraphObservationMixin:
    ...
    def _extend_observation_space(self):
        ...
        if not hasattr(self, "observation_space") or not isinstance(
        ...
        graph_spaces = {
            ...
            "graph_node_mask": Box(
                low=0.0, high=1.0, shape=(N_MAX_NODES,), dtype=np.float32
            ),
            "graph_edge_mask": Box(
                low=0.0, high=1.0, shape=(E_MAX_EDGES,), dtype=np.float32
            ),
        }
    ...
    def _get_graph_state_signature(self) -> Tuple:
        ...
        sim_frame = (
            getattr(self.nplay_headless.sim, "frame", None)
            if hasattr(self, "nplay_headless")
            else None
        )
                ...
                entities.append(
                    {
                        ...
                        "state": 1.0
                        if self.nplay_headless.exit_switch_activated()
                        else 0.0,
                    }
                )
            except Exception:
