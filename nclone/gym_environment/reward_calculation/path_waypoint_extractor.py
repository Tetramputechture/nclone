...
class PathWaypointExtractor:
    ...
    def _extract_waypoints_from_path(
        ...
        sharp_turns = self._detect_sharp_turns(path_nodes, phase, min_angle=90.0)
        waypoints.extend(sharp_turns)
        waypoints.extend(self._detect_direction_reversals(path_nodes, phase))

        # Pass 2: Strategic waypoints (medium turns, segment midpoints)
        medium_turns = self._detect_medium_turns(
        ...
        waypoints.extend(
            self._add_turn_continuation_waypoints(
                path_nodes, sharp_turns + medium_turns, phase, min_turn_angle=60.0
            )
        )
        waypoints.extend(
    ...
    def _detect_sharp_turns(
        ...
        for i in range(1, len(path_nodes) - 1):
            ...
            if angle >= min_angle:
                ...
                logger.debug(f"Sharp turn ({angle:.1f}°) at node {i}: {curr_node}")
    ...
    def _detect_medium_turns(
        ...
        for i in range(1, len(path_nodes) - 1):
            ...
            if min_angle <= angle < max_angle:
                ...
                logger.debug(f"Medium turn ({angle:.1f}°) at node {i}: {curr_node}")
    ...
    def _add_turn_continuation_waypoints(
        self,
        path_nodes: List[Tuple[int, int]],
        turn_waypoints: List[PathWaypoint],
        phase: str,
        min_turn_angle: float = 60.0,
        continuation_distance: float = 25.0,
    ) -> List[PathWaypoint]:
        """Add continuation waypoints after significant turns to guide through trajectory changes.

        The problem: Waypoints at turns guide the agent TO the turn, but not THROUGH it.
        After collecting a turn waypoint, the agent needs guidance to continue along
        the new direction, not just stop at the turn point.

        This method adds waypoints 25px AFTER each significant turn (>60°) to provide
        continuous guidance through the trajectory change.

        Args:
            path_nodes: Path node positions
            turn_waypoints: List of turn waypoints (sharp + medium turns)
            phase: Waypoint phase
            min_turn_angle: Minimum turn angle to add continuation waypoint (degrees)
            continuation_distance: Distance after turn to place continuation waypoint (pixels)

        Returns:
            List of turn continuation waypoints
        """
        waypoints = []

        # Filter turn waypoints to those with significant curvature
        significant_turns = [
            wp for wp in turn_waypoints if wp.curvature >= min_turn_angle
        ]

        for turn_wp in significant_turns:
            turn_node_idx = turn_wp.node_index

            # Find a node approximately continuation_distance AFTER the turn
            # Walk forward along path until we've accumulated enough distance
            cumulative_dist = 0.0
            continuation_node_idx = None

            for i in range(turn_node_idx + 1, len(path_nodes)):
                prev_node = path_nodes[i - 1]
                curr_node = path_nodes[i]
                ...
                cumulative_dist += edge_length

                if cumulative_dist >= continuation_distance:
                    continuation_node_idx = i
                    break

            # Add continuation waypoint if we found a suitable node
            if continuation_node_idx is not None:
                cont_node = path_nodes[continuation_node_idx]

                waypoints.append(
                    PathWaypoint(
                        position=(
                            float(cont_node[0]) + NODE_WORLD_COORD_OFFSET,
                            float(cont_node[1]) + NODE_WORLD_COORD_OFFSET,
                        ),
                        waypoint_type="turn_continuation",
                        value=1.4,  # High value to guide through turn
                        phase=phase,
                        node_index=continuation_node_idx,
                        physics_state="unknown",
                        curvature=0.0,
                    )
                )
                ...
                logger.debug(
                    f"Turn continuation waypoint at node {continuation_node_idx} "
                    f"(25px after turn at node {turn_node_idx}, curvature={turn_wp.curvature:.1f}°)"
                )
                ...
                if len(segment_nodes) >= 3:  # Need at least 3 nodes for midpoint
                    ...
                    if segment_length > self.segment_min_length:
                        ...
                        waypoints.append(
                            PathWaypoint(
                                position=(
                                    float(midpoint_node[0]) + NODE_WORLD_COORD_OFFSET,
                                    float(midpoint_node[1]) + NODE_WORLD_COORD_OFFSET,
                                ),
