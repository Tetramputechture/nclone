...
class AdaptiveMultiPathPBRS(PBRSCalculator):
    ...
    def calculate_multipath_potential(
        self,
        state: Dict[str, Any],
        ...
        if len(candidate_paths) != len(path_confidences):
            logger.warning(
                "Mismatch between candidate paths and confidences, using standard PBRS"
            )
            standard_potential = self.calculate_combined_potential(
                state, adjacency, level_data, graph_data, scale_factor=scale_factor
            )
        ...
        for i, (path_info, base_confidence) in enumerate(
            zip(candidate_paths, path_confidences)
        ):
            ...
            path_uncertainty = self._calculate_path_uncertainty(
                path_id, adjusted_confidence
            )
        ...
        if individual_potentials:
            dominant_path = max(
                individual_potentials, key=lambda p: p.potential_value * p.confidence
            )
    ...
    def update_path_rewards(self, level_outcomes: Dict[str, Any]) -> None:
        ...
        for path_info, outcome in zip(attempted_paths, path_outcomes):
            ...
            ) * current_quality + self.learning_rate * quality_score
            ...
            new_success_rate = (
                1 - self.learning_rate
            ) * current_success_rate + self.learning_rate * (
                1.0 if path_success else 0.0
            )
    ...
    def _calculate_path_specific_potential(
        self,
        state: Dict[str, Any],
        waypoints: List[Tuple[int, int]],
        adjacency: Dict[Tuple[int, int], List[Tuple[Tuple[int, int], float]]],
        ...
        if not surface_area:
            ...
            surface_area = self._compute_reachable_surface_area(
    ...
    def _adjust_confidence_with_learning(
        self,
        path_id: str,
    ...
    def _calculate_outcome_quality_score(
        self,
    ...
    def load_adaptive_state(self, filepath: str) -> None:
        ...
        try:
            ...
            self.path_usage_counts = defaultdict(
                int, state.get("path_usage_counts", {})
            )
            self.path_success_rates = defaultdict(
                float, state.get("path_success_rates", {})
            )
