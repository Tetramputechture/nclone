...
from ..reward_calculation.navigation_reward_calculator import NavigationRewardCalculator
from ..reward_calculation.exploration_reward_calculator import ExplorationRewardCalculator
...
class TestSimplifiedRewardSystem(unittest.TestCase):
    ...
    def test_switch_activation_reward(self):
        ...
        expected_min = RewardCalculator.SWITCH_ACTIVATION_REWARD + RewardCalculator.TIME_PENALTY
    ...
    def test_exit_completion_reward(self):
        ...
        expected_min = RewardCalculator.EXIT_COMPLETION_REWARD + RewardCalculator.TIME_PENALTY
        self.assertGreaterEqual(reward, expected_min)
...
class TestPBRSPotentials(unittest.TestCase):
    ...
    def test_switch_focus_when_inactive(self):
        ...
        potential = self.pbrs_calculator.calculate_combined_potential(self.state_switch_inactive)
        
        # Should be positive (closer to switch = higher potential)
        self.assertGreater(potential, 0.0)
        
        # Should use switch distance scaling
        objective_pot = PBRSPotentials.objective_distance_potential(self.state_switch_inactive)
    ...
    def test_exit_focus_when_active(self):
        ...
        potential = self.pbrs_calculator.calculate_combined_potential(self.state_switch_active)
        
        # Should be positive (closer to exit = higher potential)
        self.assertGreater(potential, 0.0)
        
        # Should use exit distance scaling
        objective_pot = PBRSPotentials.objective_distance_potential(self.state_switch_active)
    ...
    def test_disabled_potentials(self):
        ...
        components = self.pbrs_calculator.get_potential_components(self.state_switch_inactive)
    ...
    def test_objective_distance_potential(self):
        ...
        # Test switch phase
        potential_switch = PBRSPotentials.objective_distance_potential(self.state_switch_inactive)
        self.assertGreater(potential_switch, 0.0)
        self.assertLessEqual(potential_switch, 1.0)
        
        # Test exit phase
        potential_exit = PBRSPotentials.objective_distance_potential(self.state_switch_active)
        self.assertGreater(potential_exit, 0.0)
        self.assertLessEqual(potential_exit, 1.0)


class TestNavigationRewardCalculator(unittest.TestCase):
    """Test the navigation reward calculator."""

    def setUp(self):
        """Set up test fixtures."""
        self.nav_calculator = NavigationRewardCalculator()
    ...
    def test_no_switch_activation_reward(self):
        """Test that switch activation reward is disabled (handled by main calculator)."""
        self.assertEqual(NavigationRewardCalculator.SWITCH_ACTIVATION_REWARD, 0.0)

    def test_distance_improvement_reward(self):
        """Test that moving closer to objectives gives positive reward."""
        # First call to establish baseline
        reward1, _ = self.nav_calculator.calculate_navigation_reward(self.state_far, self.state_far)
        
        # Second call with closer position should give positive reward
        reward2, _ = self.nav_calculator.calculate_navigation_reward(self.state_closer, self.state_far)
        
        # Should get positive reward for getting closer
        self.assertGreater(reward2, reward1)

    def test_potential_calculation(self):
        """Test potential calculation for navigation."""
        potential = self.nav_calculator.calculate_potential(self.state_far)
        
        # Should be a small value (can be negative when far from objective)
        self.assertIsInstance(potential, (int, float, np.number))
        self.assertLess(abs(potential), 0.1)  # Should be small compared to main rewards
