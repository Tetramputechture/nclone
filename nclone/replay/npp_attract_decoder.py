...
class NppAttractDecoder:
    ...
    def decode_npp_attract_file(self, npp_attract_path: str) -> Dict:
        ...
        try:
            ...
            logger.info(f"Processing npp_attract file: {npp_attract_path} ({len(npp_data)} bytes)")
            ...
            logger.info(f"Successfully decoded: {len(tiles)} tiles, {len(entities)} entities, spawn {ninja_spawn}")
            return result
    ...
    def create_nclone_map(self, npp_attract_path: str) -> bytes:
        ...
        try:
            ...
            if len(complete_map) != expected_size:
                logger.warning(f"Map size mismatch: {len(complete_map)} != {expected_size}")
    ...
    def validate_against_reference(self, npp_attract_path: str, reference_map_path: str) -> Dict[str, float]:
        """
        ...
        try:
            ...
            tile_matches = sum(1 for i in range(966) if decoded_data['tiles'][i] == ref_tiles[i])
            tile_accuracy = tile_matches / 966
            
            # Calculate spawn accuracy
            spawn_accuracy = 1.0 if decoded_data['ninja_spawn'] == ref_ninja_spawn else 0.0
            
            # Calculate entity accuracy
            entity_accuracy = self._calculate_entity_accuracy(decoded_data['entities'], ref_entities)
            ...
            results = {
                ...
                'total_tiles': 966,
                'entity_matches': int(entity_accuracy * len(ref_entities)) if ref_entities else 0,
                'total_entities': len(ref_entities)
            }
            
            logger.info(f"Validation results: Tiles {tile_accuracy:.1%}, Entities {entity_accuracy:.1%}, Spawn {spawn_accuracy:.1%}")
            return results
            
        except Exception as e:
            logger.error(f"Error validating {npp_attract_path} against {reference_map_path}: {e}")
            return {
                'tile_accuracy': 0.0,
                'entity_accuracy': 0.0,
                'spawn_accuracy': 0.0,
    ...
    def _calculate_entity_accuracy(self, decoded_entities: List[Dict], ref_entities: List[Dict]) -> float:
        ...
        for i in range(min(len(decoded_entities), len(ref_entities))):
            ...
            if (decoded['type'] == reference['type'] and
                decoded['x'] == reference['x'] and
                decoded['y'] == reference['y'] and
                decoded['orientation'] == reference['orientation'] and
                decoded['mode'] == reference['mode']):
                matches += 1
