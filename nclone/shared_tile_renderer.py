"""
Shared tile rendering logic for consistent tile rendering across the application.

This module provides a unified tile rendering system that can be used by both
...
class SharedTileRenderer:
    """Shared tile rendering logic that can be used across different rendering contexts."""
    
    @staticmethod
    def render_tiles_to_pygame_surface(tile_dic, surface_width, surface_height, tile_size=24, tile_color_rgb=None):
        """
        Render tiles to a pygame surface using the same logic as tile_renderer.py.
        ...
        for tile_type, coords_list in tile_groups.items():
            if tile_type == 1 or tile_type > 33:
                ...
                for x, y in coords_list:
                    SharedTileRenderer._draw_complex_tile(ctx, tile_type, x, y, tile_size)
    ...
    @staticmethod
    def _draw_complex_tile(ctx, tile_type, x, y, tile_size):
        """Draw a complex tile shape using the same logic as tile_renderer.py."""
        if tile_type < 10:
