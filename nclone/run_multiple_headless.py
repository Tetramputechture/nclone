...
def run_single_simulation(simulation_id, num_steps, video_path_prefix=None):
    ...
    print(
        f"Simulation {simulation_id} finished {num_steps} steps in {end_time - start_time:.2f} seconds."
    )

    if video_path_prefix and frames:
        video_filename = f"{video_path_prefix}{simulation_id}.mp4"
        print(
            f"Simulation {simulation_id}: Saving video to {video_filename} ({len(frames)} frames)..."
        )
        try:
...
def main():
    parser = argparse.ArgumentParser(
        description="Run multiple N++ headless simulations concurrently."
    )
    parser.add_argument(
        "--num-simulations",
        type=int,
    ...
    print(
        f"Starting {args.num_simulations} headless simulations, each for {args.num_steps} steps..."
    )
    if video_path_prefix:
    ...
    for i in range(args.num_simulations):
        p = multiprocessing.Process(
            target=run_single_simulation, args=(i, args.num_steps, video_path_prefix)
        )
        processes.append(p)
        p.start()
