...
class KeyboardController:
    ...
    def register_key(
        self, key: int, handler: Callable, modifiers: Optional[tuple] = None
    ):
...
def setup_default_controls(
    controller: KeyboardController, env, config, state: Dict
) -> KeyboardController:
    ...
    def handle_reset():
        ...
        if state.get("generator_tester"):
            try:
                ...
                print(
                    f"Generated new map: {state['generator_tester'].get_info_string()}"
                )
    ...
    def toggle_collision():
        state["collision_debug_enabled"] = not state.get(
    ...
    def toggle_interactive_graph():
        state["interactive_graph_enabled"] = not state.get(
            "interactive_graph_enabled", False
        )
        print(
            f"Interactive graph: {'ON' if state['interactive_graph_enabled'] else 'OFF'}"
        )
    ...
    if config.test_generators:
        ...
        def toggle_ascii():
            ...
            print(
                f"ASCII visualization: {'ON' if state['show_ascii_on_reset'] else 'OFF'}"
            )
        ...
        for num_key, gen_idx in {
            ...
            controller.register_key(
                num_key,
                lambda idx=gen_idx: state.get("generator_tester")
                and state["generator_tester"].jump_to_generator(idx),
            )
    ...
    if config.visualize_reachability or config.reachability_from_ninja:
        ...
        def toggle_reachability():
            state["reachability_debug_enabled"] = not state.get(
                "reachability_debug_enabled", False
            )
            print(
                f"Reachability: {'ON' if state['reachability_debug_enabled'] else 'OFF'}"
            )
    ...
    if config.test_path_aware:
        ...
        def toggle_path_distances():
            if not state.get("subgoal_debug_enabled"):
                state["path_distances_debug_enabled"] = not state.get(
                    "path_distances_debug_enabled", False
                )
                print(
                    f"Path distances: {'ON' if state['path_distances_debug_enabled'] else 'OFF'}"
                )

        def toggle_adjacency_graph():
            state["adjacency_graph_debug_enabled"] = not state.get(
                "adjacency_graph_debug_enabled", False
            )
            print(
                f"Adjacency graph: {'ON' if state['adjacency_graph_debug_enabled'] else 'OFF'}"
            )

        def toggle_blocked_entities():
            if not state.get("recorder"):
                state["blocked_entities_debug_enabled"] = not state.get(
                    "blocked_entities_debug_enabled", False
                )
                print(
                    f"Blocked entities: {'ON' if state['blocked_entities_debug_enabled'] else 'OFF'}"
                )
