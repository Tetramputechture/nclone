...
from nclone.nclone_environments.basic_level_no_gold.reward_calculation.pbrs_potentials import (
    PBRSPotentials, PBRSCalculator
)
...
class TestPBRSPotentials(unittest.TestCase):
    ...
    def test_exploration_potential(self):
        ...
        potential = PBRSPotentials.exploration_potential(self.test_state, visited_positions)
        ...
        visited_potential = PBRSPotentials.exploration_potential(visited_state, visited_positions)
...
class TestPBRSIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.env_pbrs = BasicLevelNoGold(
        ...
        self.env_no_pbrs = BasicLevelNoGold(
            render_mode='rgb_array',
            enable_frame_stack=False,
            observation_profile='rich',
            enable_pbrs=False
    ...
    def test_pbrs_reward_shaping(self):
        ...
        for action in [1, 2, 1, 0, 2]:  # Some movement actions
            _, reward_pbrs, term_pbrs, trunc_pbrs, info_pbrs = self.env_pbrs.step(action)
            _, reward_normal, term_normal, trunc_normal, info_normal = self.env_no_pbrs.step(action)
