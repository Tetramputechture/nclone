...
try:
    ...
    # Fallback to numpy for environments without torch
    class torch:
        @staticmethod
...
class FeatureConfig:
    ...
    def __post_init__(self):
        ...
        expected_total = (self.objective_slots + self.switch_slots + self.hazard_slots + 
                         self.area_slots + self.movement_slots + self.meta_slots)
        if expected_total != self.total_features:
            raise ValueError(f"Feature slots sum to {expected_total}, expected {self.total_features}")
...
class CompactReachabilityFeatures:
    ...
    def encode_reachability(
        ...
        try:
            ...
            if self.debug:
                print(f"DEBUG: Feature encoding error: {e}")
    ...
    def _encode_objective_distances(
        ...
        """
        distances = np.ones(self.config.objective_slots, dtype=np.float32)  # Default: unreachable
        ...
        try:
            ...
            if current_objective:
                ...
                if self.debug:
                    print(f"DEBUG: Current objective: {current_objective.objective_type.value} "
            ...
            for i, objective_info in enumerate(other_objectives[:self.config.objective_slots-1], 1):
                if self._is_position_reachable(objective_info['position'], reachability_result):
                    raw_distance = self._calculate_distance(ninja_position, objective_info['position'])
            ...
            if self.debug:
                print(f"DEBUG: Objective distance encoding error: {e}")
            # Return default distances on error
    ...
    def _encode_switch_states(
        ...
        try:
            ...
            for i, switch_info in enumerate(switches[:self.config.switch_slots]):
                ...
                elif position and self._is_position_reachable(position, reachability_result):
                ...
                if self._switch_has_dependencies(switch_info):
                    dependency_bonus = min(0.1 * self._count_switch_dependencies(switch_info), 0.4)
                    switch_features[i] = min(switch_features[i] + dependency_bonus, 1.4)
                    
        except Exception as e:
            if self.debug:
                print(f"DEBUG: Switch state encoding error: {e}")
    ...
    def _encode_hazard_proximities(
        ...
        try:
            ...
            if self.debug:
                print(f"DEBUG: Hazard proximity encoding error: {e}")
    ...
    def _encode_area_connectivity(
        ...
        try:
            ...
            if self.debug:
                print(f"DEBUG: Area connectivity encoding error: {e}")
    ...
    def _encode_movement_capabilities(
        ...
        try:
            ...
            if self.debug:
                print(f"DEBUG: Movement capability encoding error: {e}")
    ...
    def _encode_meta_features(
        ...
        try:
            ...
            if hasattr(level_data, 'shape'):
                ...
                total_positions = getattr(level_data, 'width', 100) * getattr(level_data, 'height', 100)
            ...
            method_encoding = {'flood_fill': 0.3, 'tier1': 0.3, 'tier2': 0.6, 'tier3': 1.0}
            ...
            if self.debug:
                print(f"DEBUG: Meta-feature encoding error: {e}")
    ...
    def _is_position_reachable(self, position: Tuple[float, float], reachability_result: ReachabilityResult) -> bool:
    ...
    def _identify_other_objectives(self, entities: List[Any], current_objective: Optional[SimpleObjective]) -> List[Dict[str, Any]]:
    ...
    def _define_level_sectors(self, level_data: Any, ninja_position: Tuple[float, float]) -> List[Dict[str, Any]]:
    ...
    def _estimate_level_complexity(self, level_data: Any, entities: List[Any]) -> float:
        ...
        switch_count = len([e for e in entities if hasattr(e, 'entity_type') and 'switch' in e.entity_type])
        complexity += min(switch_count / 10.0, 0.3)  # Max 0.3 from switches
        
        # Factor 3: Hazard count
        hazard_count = len([e for e in entities if hasattr(e, 'entity_type') and self._is_hazard_type(e.entity_type)])
        ...
        if hasattr(level_data, 'shape'):
            ...
            level_size = getattr(level_data, 'width', 100) * getattr(level_data, 'height', 100)
    ...
    def _calculate_hazard_density(self, entities: List[Any], level_data: Any) -> float:
        ...
        if hasattr(level_data, 'shape'):
            ...
            level_area = getattr(level_data, 'width', 100) * getattr(level_data, 'height', 100)
        
        density = hazard_count / (level_area / 100.0)  # Hazards per 100 tiles
        return min(density, 1.0)
    
    def _debug_feature_summary(self, features: np.ndarray, ninja_position: Tuple[float, float]):
    ...
    def get_feature_names(self) -> List[str]:
        ...
        # Meta features [56-63]
        meta_names = ['confidence', 'computation_time', 'level_complexity', 'reachable_ratio',
                     'switch_complexity', 'hazard_density', 'analysis_method', 'cache_hit']
