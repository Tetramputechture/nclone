...
class RLIntegrationAPI:
    ...
    def get_rl_state(
        ...
        cache_hit = (
            hasattr(reachability_state, "_cache_hit") and reachability_state._cache_hit
        )
    ...
    def calculate_curiosity_reward(
        self, rl_state: RLState, position: Tuple[float, float], radius: float = 3.0
    ) -> float:
        """
        ...
        if (
            0 <= sub_row < rl_state.curiosity_map.shape[0]
            and 0 <= sub_col < rl_state.curiosity_map.shape[1]
        ):
    ...
    def plan_level_completion_path(
        self, rl_state: RLState, current_position: Tuple[float, float]
    ) -> List:
        """
        ...
        critical_subgoals = [
            s for s in rl_state.subgoals if s.goal_type in {"exit_switch", "exit_door"}
        ]

        # Get supporting subgoals (doors, switches)
        supporting_subgoals = [
            s
            for s in rl_state.subgoals
    ...
    def get_reachability_features(
        self, rl_state: RLState, position: Tuple[float, float], feature_radius: int = 5
    ) -> Dict[str, float]:
        """
        ...
        features["is_reachable"] = float(
            (sub_row, sub_col) in rl_state.reachable_positions
        )
        features["reachable_area_size"] = float(len(rl_state.reachable_positions))

        # Curiosity and exploration features
        features["curiosity_value"] = self.calculate_curiosity_reward(
            rl_state, position
        )
        features["num_nearby_frontiers"] = float(
            self._count_nearby_frontiers(
        ...
        features["num_critical_subgoals"] = float(
            len(
                [
                    s
                    for s in rl_state.subgoals
        ...
        if (
            0 <= sub_row < rl_state.accessibility_map.shape[0]
            and 0 <= sub_col < rl_state.accessibility_map.shape[1]
        ):
            features["accessibility_score"] = float(
                rl_state.accessibility_map[sub_row, sub_col]
            )
        else:
    ...
    def _generate_curiosity_map(
        self, level_data, frontiers: List[Frontier]
    ) -> np.ndarray:
                    ...
                    for dc in range(-2, 3):
                        ...
                        if 0 <= adj_row < height and 0 <= adj_col < width:
                            ...
                            if distance > 0:
                                ...
                                spread_value = (
                                    frontier.exploration_value * decay_factor * 0.3
                                )
    ...
    def _generate_accessibility_map(
        self, level_data, reachable_positions: Set[Tuple[int, int]]
    ) -> np.ndarray:
        ...
        centroid_row = sum(pos[0] for pos in reachable_positions) / len(
            reachable_positions
        )
        centroid_col = sum(pos[1] for pos in reachable_positions) / len(
            reachable_positions
        )
        ...
        for row, col in reachable_positions:
            ...
            if 0 <= row < height and 0 <= col < width:
                distance = math.sqrt(
                    (row - centroid_row) ** 2 + (col - centroid_col) ** 2
                )
    ...
    def _count_nearby_frontiers(
        self, frontiers: List[Frontier], position: Tuple[int, int], radius: int
    ) -> int:
    ...
    def _distance_to_nearest_subgoal(
        self, subgoals: List, position: Tuple[int, int]
    ) -> float:
