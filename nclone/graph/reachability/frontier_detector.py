...
class FrontierDetector:
    ...
    def detect_frontiers(
        ...
        if self.debug:
            ...
            for i, frontier in enumerate(self.frontiers[:5]):  # Show top 5
                print(
                    f"  {i + 1}. {frontier.frontier_type.value} at {frontier.position} "
    ...
    def _find_frontier_candidates(
        self, reachable_positions: Set[Tuple[int, int]], tiles, height: int, width: int
    ) -> Set[Tuple[int, int]]:
    ...
    def _analyze_blocking_reasons(
        ...
        for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if (
                0 <= adj_row < height
                and 0 <= adj_col < width
                and (adj_row, adj_col) not in reachable_positions
            ):
                ...
                if level_data.tiles[adj_row, adj_col] != 0:
                    ...
                    if hazard_extension and position_validator:
                        ...
                        if not hazard_extension.is_position_safe_for_reachability(
                            (pixel_x, pixel_y)
                        ):
    ...
    def _determine_frontier_type(
        self, blocking_reasons: Dict[str, int]
    ) -> FrontierType:
        """Determine frontier type based on blocking reasons."""
        ...
        while queue and area_size < max_area:
            ...
            for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                ...
                if (
                    ...
                    and (next_row, next_col) not in reachable_positions
    ...
    def _calculate_exploration_values(
    ...
    def calculate_curiosity_bonus(
        self, position: Tuple[int, int], radius: int = 3
    ) -> float:
        """
    ...
    def get_exploration_statistics(self) -> Dict[str, any]:
        ...
        for frontier_type in FrontierType:
            type_counts[frontier_type.value] = len(
                self.get_frontiers_by_type(frontier_type)
            )

        return {
            "total_frontiers": len(self.frontiers),
            "type_distribution": type_counts,
            "avg_exploration_value": sum(f.exploration_value for f in self.frontiers)
            / len(self.frontiers),
            "avg_accessibility_score": sum(
                f.accessibility_score for f in self.frontiers
            )
            / len(self.frontiers),
