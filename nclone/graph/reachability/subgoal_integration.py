...
class ReachabilitySubgoalIntegration:
    ...
    def enhance_subgoals_with_reachability(
        ...
        if self.debug:
            print(
                f"DEBUG: Enhanced {len(existing_subgoals)} existing subgoals to {len(enhanced_subgoals)} total"
            )

        return enhanced_subgoals

    def _identify_strategic_waypoints(
        self, level_data, reachability_state, position_validator=None
    ):
        """
    ...
    def _is_strategic_position(
        self, row: int, col: int, level_data, reachable_positions
    ) -> bool:
        """
        ...
        for dr in [-1, 0, 1]:
            for dc in [-1, 0, 1]:
                ...
                # Check bounds
                if (
                    0 <= neighbor_row < level_data.tiles.shape[0]
                    and 0 <= neighbor_col < level_data.tiles.shape[1]
                ):
        ...
        return reachable_neighbors >= 3 or (
            reachable_neighbors >= 2 and unreachable_neighbors >= 2
        )
