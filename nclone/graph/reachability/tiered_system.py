...
class TieredReachabilitySystem:
    ...
    def analyze_reachability(
        ...
        try:
            ...
            if self.debug:
                print(f"DEBUG: Tier {tier_to_use} failed: {e}")
            
            # Fallback to next tier on failure
            if tier_to_use < 3:
                return self.analyze_reachability(
                    level_data, ninja_position, switch_states, PerformanceTarget.ACCURATE
                )
            else:
                raise e
        ...
        for tier in [1, 2, 3]:
            ...
            if history['times']:
                summary[f'tier{tier}'] = {
                    ...
                    'avg_accuracy': np.mean(history['accuracies']) if history['accuracies'] else 0.0,
                    'min_accuracy': np.min(history['accuracies']) if history['accuracies'] else 0.0,
