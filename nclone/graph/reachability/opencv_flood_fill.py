...
class OpenCVFloodFill:
    ...
    def quick_check(
        ...
        try:
            ...
            collision_mask = self._create_collision_mask(level_data, switch_states, entities)
            ...
            if self.debug:
                ...
                self._save_debug_images(collision_mask, traversable_mask, reachable_mask, ninja_pos)
            ...
            if self.debug:
                print(f"DEBUG: OpenCV flood fill failed after {elapsed_ms:.3f}ms: {e}")
            
            # Return minimal result on failure
            return ReachabilityApproximation(
                reachable_positions={ninja_pos},
                confidence=0.1,
                computation_time_ms=elapsed_ms,
                method="opencv_flood_fill_fallback",
                tier_used=0
            )
    ...
    def _render_entity_collisions(
        ...
        entity_renderer = EntityRenderer(
            mock_sim, mock_screen, self.render_scale,
            mock_screen.get_width(), mock_screen.get_height()
        )
    ...
    def _is_entity_solid_for_rendering(self, entity: Any, switch_states: Dict[str, bool]) -> bool:
        """
    ...
    def _mask_to_positions(self, reachable_mask: np.ndarray) -> Set[Tuple[int, int]]:
        ...
        valid_mask = (
            (center_x >= 0) & (center_x < level_width_px) &
            (center_y >= 0) & (center_y < level_height_px)
        )
        ...
        if self.debug:
            print(f"DEBUG: Generated {len(positions)} tile center positions from {len(y_coords)} pixels")
    ...
    def _save_debug_images(
        ...
        try:
            ...
            ninja_radius = max(2, int(10 * self.render_scale))  # Scale 10px ninja radius with render scale
            if (0 <= ninja_x < reachable_debug.shape[1] and 
                0 <= ninja_y < reachable_debug.shape[0]):
