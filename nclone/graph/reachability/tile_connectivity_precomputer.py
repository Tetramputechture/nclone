...
class TileConnectivityPrecomputer:
    ...
    def precompute_all(self, verbose: bool = True) -> Dict[Tuple[int, int, str], bool]:
        ...
        if verbose:
            print(
                f"Precomputing {total_combinations} tile connectivity combinations..."
            )
            print(
                f"  (Tiles 0-{self.VALID_TILES - 1} only, excluding glitched tiles 34-37)"
            )
            print()
        ...
        for tile_a in range(self.VALID_TILES):
            for tile_b in range(self.VALID_TILES):
                for dir_name, dir_vec in self.directions.items():
                    ...
                    can_traverse = self._check_traversability(tile_a, tile_b, dir_vec)
                    ...
                    if verbose and processed % 1000 == 0:
                        ...
                        remaining = (
                            (total_combinations - processed) / rate if rate > 0 else 0
                        )
                        print(
        ...
        if verbose:
            print()
            print(
                f"Precomputation complete in {elapsed:.1f}s: {total_combinations} combinations"
            )

        return traversability

    def _check_traversability(
        self, tile_a: int, tile_b: int, direction: Tuple[int, int]
    ) -> bool:
        """
        ...
        has_blocking_b = self._has_blocking_segments_in_direction(
            tile_b, tuple(-d for d in direction)
        )
    ...
    def _has_blocking_segments_in_direction(
        self, tile_id: int, direction: Tuple[int, int]
    ) -> bool:
        """
        ...
        if ortho_data:
            ...
            if dx == 1 and dy == 0:  # East
                ...
                if len(ortho_data) > 11 and (
                    ortho_data[10] != 0 or ortho_data[11] != 0
                ):
                    return True
    ...
    def save_to_file(self, traversability: Dict, filepath: str, verbose: bool = True):
        ...
        if verbose:
            ...
            total = compact.size
            print(
                f"  Traversable combinations: {traversable_count:,}/{total:,} "
