...
class GraphVisualizationAPI:
    ...
    def visualize_graph(self, request: VisualizationRequest) -> VisualizationResult:
        ...
        try:
            ...
            surface = self._create_visualization_surface(
                request, graph_data, path_result
            )
    ...
    def create_interactive_session(
        ...
        self.graph_constructor.build_sub_cell_graph(
            level_data,
            ninja_position,
            ninja_velocity,
            ninja_state,
    ...
    def _get_graph_data(self, request: VisualizationRequest) -> Optional[GraphData]:
        ...
        # Check cache
        level_id = id(request.level_data)
    ...
    def _create_visualization_surface(
        ...
        if request.mode == VisualizationMode.STANDALONE:
            ...
            sim_surface = pygame.Surface(
                (request.output_size[0] // 2, request.output_size[1])
            )
            sim_surface.fill((50, 50, 50))  # Dark background
    ...
    def _find_closest_node(
        ...
        for node_idx in range(graph_data.num_nodes):
            ...
            if node_idx < sub_grid_nodes_count:
                ...
                node_x = sub_col * SUB_CELL_SIZE + SUB_CELL_SIZE * 0.5 + TILE_PIXEL_SIZE
                node_y = sub_row * SUB_CELL_SIZE + SUB_CELL_SIZE * 0.5 + TILE_PIXEL_SIZE
            else:
                # Entity node: extract from features
                ...
                if len(node_features) > state_offset + 2:
                    ...
                    node_x = norm_x * float(FULL_MAP_WIDTH_PX)
                    node_y = norm_y * float(FULL_MAP_HEIGHT_PX)
                else:
                    continue
    ...
    def _calculate_graph_stats(
        self, graph_data: GraphData, path_result: Optional[PathResult]
    ) -> Dict[str, Any]:
        ...
        for node_idx in range(graph_data.num_nodes):
            if graph_data.node_mask[node_idx] > 0:
                ...
                node_type_counts[node_type.name] = (
                    node_type_counts.get(node_type.name, 0) + 1
                )
        ...
        for edge_idx in range(graph_data.num_edges):
            if graph_data.edge_mask[edge_idx] > 0:
                ...
                edge_type_counts[edge_type.name] = (
                    edge_type_counts.get(edge_type.name, 0) + 1
                )
