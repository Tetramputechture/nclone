...
class SimplifiedCompletionStrategy:
    ...
    def get_next_objective(
        ...
        if exit_switch and self._is_reachable(
            ninja_position, exit_switch["position"], level_data, switch_states
        ):
            objective = SimpleObjective(
                objective_type=ObjectiveType.REACH_EXIT_SWITCH,
                position=exit_switch["position"],
                distance=self._calculate_distance(
                    ninja_position, exit_switch["position"]
                ),
                priority=1.0,
        ...
        if exit_switch:  # Exit switch exists but not reachable
            door_switch = self._find_nearest_reachable_door_switch(
                ninja_position, entities, level_data, switch_states
            )
            if door_switch:
                objective = SimpleObjective(
                    ...
                    distance=self._calculate_distance(
                        ninja_position, door_switch["position"]
                    ),
                    priority=0.9,
        ...
        if exit_door and self._is_reachable(
            ninja_position, exit_door["position"], level_data, switch_states
        ):
            objective = SimpleObjective(
                objective_type=ObjectiveType.REACH_EXIT_DOOR,
                position=exit_door["position"],
                distance=self._calculate_distance(
                    ninja_position, exit_door["position"]
                ),
                priority=1.0,
        ...
        any_switch = self._find_nearest_reachable_switch(
            ninja_position, entities, level_data, switch_states
        )
        if any_switch:
            objective = SimpleObjective(
                ...
                distance=self._calculate_distance(
                    ninja_position, any_switch["position"]
                ),
                priority=0.5,
    ...
    def _is_reachable(
        ...
        if self.debug:
            print(
                f"DEBUG: Checking reachability from {ninja_position} to {target_position}"
            )
            print(f"DEBUG: Target tile: {target_tile}, Target pixel: {target_pixel}")
            print(
                f"DEBUG: Reachable positions count: {len(result.reachable_positions)}"
            )
    ...
    def is_objective_reached(
        ...
        distance = self._calculate_distance(
            ninja_position, self.current_objective.position
        )
    ...
    def get_objective_for_rl_features(
        ...
        type_features = {
            "objective_type_exit_switch": 1.0
            if self.current_objective.objective_type == ObjectiveType.REACH_EXIT_SWITCH
            else 0.0,
            "objective_type_door_switch": 1.0
            if self.current_objective.objective_type == ObjectiveType.REACH_DOOR_SWITCH
            else 0.0,
            "objective_type_exit_door": 1.0
            if self.current_objective.objective_type == ObjectiveType.REACH_EXIT_DOOR
            else 0.0,
            "objective_type_switch": 1.0
            if self.current_objective.objective_type == ObjectiveType.REACH_SWITCH
            else 0.0,
        }
