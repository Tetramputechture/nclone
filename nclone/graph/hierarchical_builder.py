...
class HierarchicalGraphBuilder:
    ...
    def _build_sub_cell_graph(
        ...
        """
        # Initialize arrays
        node_features = np.zeros((N_MAX_NODES, self.base_node_feature_dim), dtype=np.float32)
        edge_index = np.zeros((2, E_MAX_EDGES), dtype=np.int32)
        edge_features = np.zeros((E_MAX_EDGES, self.base_edge_feature_dim), dtype=np.float32)
    ...
    def _extract_sub_cell_features(
        ...
        features = np.zeros(self.base_node_feature_dim, dtype=np.float32)
    ...
    def _extract_edge_features(
        ...
        features = np.zeros(self.base_edge_feature_dim, dtype=np.float32)
                ...
                for j, (other_idx, other_entity) in enumerate(entity_nodes):
                    if i != j and other_entity.get('type') == 17:
                        ...
                        if distance < 50.0:  # Within chaining range
                            ...
                            chain_features = np.zeros(self.base_edge_feature_dim, dtype=np.float32)
