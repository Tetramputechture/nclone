...
class SubgoalVisualizer:
    ...
    def render_subgoals_overlay(
        ...
        for i, subgoal in enumerate(subgoals):
            self._render_single_subgoal(
                overlay, subgoal, i, ninja_position, 
                current_plan, tile_x_offset, tile_y_offset, adjust
            )
    ...
    def _render_subgoal_connections(
        ...
        for i in range(len(plan.execution_order) - 1):
            ...
            if current_idx < len(subgoals) and next_idx < len(subgoals):
                ...
                pygame.draw.line(
                    surface, self.config.connection_color, 
                    start_pos, end_pos, self.config.connection_width
                )
                
                # Draw arrow at end
                self._draw_arrow(surface, start_pos, end_pos, self.config.connection_color)
    ...
    def _render_single_subgoal(
        ...
        screen_pos = self._subgoal_to_screen_pos(subgoal, tile_x_offset, tile_y_offset, adjust)
        ...
        if self.config.animate_subgoals:
            pulse = 1.0 + 0.2 * np.sin(self.animation_time * self.config.pulse_speed + index)
    ...
    def _subgoal_to_screen_pos(
        ...
        x = sub_col * SUB_CELL_SIZE * adjust + SUB_CELL_SIZE * adjust // 2 + tile_x_offset
        y = sub_row * SUB_CELL_SIZE * adjust + SUB_CELL_SIZE * adjust // 2 + tile_y_offset
        return (int(x), int(y))
    ...
    def _draw_next_subgoal_highlight(
        ...
        pygame.draw.circle(
            surface, (255, 255, 0, 150), center, highlight_radius, 3
        )
    ...
    def export_subgoal_visualization(
        ...
        try:
            ...
            self.render_subgoals_overlay(
                export_surface, subgoals, ninja_position, 
                reachable_positions, current_plan,
