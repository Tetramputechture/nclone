#!/usr/bin/env python3
"""
Create pathfinding visualization from ninja to leftmost locked door switch.

This script creates a comprehensive visualization showing:
1. The doortest map with proper level geometry
2. A valid path from ninja starting position to leftmost locked door switch
...
def create_pathfinding_visualization():
    ...
    print("=" * 80)
    print("🎯 CREATING PATHFINDING VISUALIZATION")
    print("=" * 80)
    ...
    try:
        ...
        # List all entities to understand what we have
        print(f"\n🔍 Available entities ({len(level_data.entities)}):")
    ...
    try:
        ...
        print(f"✅ Ninja node: {ninja_node} at {ninja_coords} (distance: {min_ninja_dist:.1f}px)")
        
        # Find all locked door switches (type 6)
        ...
        for i, (x, y) in enumerate(locked_door_switches):
            print(f"  Switch {i}: ({x}, {y})")
        
        # Find the leftmost locked door switch
        ...
        print(f"✅ Target node: {target_node} at {target_coords} (distance: {min_target_dist:.1f}px)")
    ...
    try:
        ...
        for tile_y in range(level_data.height):
            for tile_x in range(level_data.width):
                ...
                if tile_value != 0:  # Solid tile
                    ...
                    # Draw as gray rectangle (solid geometry)
                    rect = Rectangle(
        ...
        ninja_circle = Circle(ninja_coords, 10, facecolor='black', edgecolor='white', linewidth=3, zorder=12)
        ax.add_patch(ninja_circle)
        
        # Add ninja label
        ax.text(ninja_coords[0] - 20, ninja_coords[1] - 25, 'ninja', 
                fontsize=12, fontweight='bold', color='black',
                bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
        
        # Draw target position
        target_circle = Circle((target_x, target_y), 10, facecolor='black', edgecolor='white', linewidth=3, zorder=12)
        ax.add_patch(target_circle)
        
        # Add target label
        ax.text(target_x - 40, target_y - 25, 'locked door switch', 
                fontsize=12, fontweight='bold', color='black',
                bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
        ...
        if len(path_coords) >= 2:
            ...
            ax.plot(path_xs, path_ys, 'r-', linewidth=4, alpha=0.7, zorder=11)
            
            # Add directional arrows along the path
            arrow_spacing = max(1, len(path_coords) // 8)  # Show ~8 arrows
            for i in range(0, len(path_coords) - 1, arrow_spacing):
                ...
                arrow = FancyArrowPatch(
                    ...
                    color='red', linewidth=3, zorder=11
                )
                ax.add_patch(arrow)
        
        # Add title and labels
        ax.set_title(f'Pathfinding Visualization: Ninja → Leftmost Locked Door Switch\n'
                    f'Path: {len(path_result.path)} nodes, Cost: {path_result.total_cost:.1f}',
...
Path Cost: {path_result.total_cost:.1f}
Algorithm: A*
...
def main():
    ...
    if success:
        print("\n" + "=" * 80)
        print("🎉 SUCCESS! Pathfinding visualization created successfully!")
