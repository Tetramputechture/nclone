#!/usr/bin/env python3
"""
Create final corrected pathfinding visualization with proper tile rendering and positioning.

This script fixes the critical issues:
1. Proper tile rendering using actual tile segment definitions
2. Correct positioning - tiles at original positions, only entities/path offset for padding
3. Accurate complex tile shapes (slopes, curves, etc.)
"""
...
def create_accurate_tile_patches(tile_value, tile_x, tile_y):
    """
    Create accurate visual representation of a tile using actual tile segment definitions.
    ...
    if tile_value == 0:
        ...
        # Half tiles - accurate positioning
        ...
        # 45-degree slopes - create accurate diagonal shapes
        ...
        # Quarter circles and pipes - use actual circular segment definitions
        if tile_value in TILE_SEGMENT_CIRCULAR_MAP:
            ...
            else:  # Quarter pipes (concave corners) - create accurate L-shapes
                if tile_value == 14:  # Top-left pipe (hollow in top-left)
                    # Create full tile minus the hollow quarter circle
                    full_rect = Rectangle((tile_x, tile_y), TILE_PIXEL_SIZE, TILE_PIXEL_SIZE, 
                                        facecolor='#C0C0C0', edgecolor='#A0A0A0', linewidth=1)
                    patches_list.append(full_rect)
                    # Add hollow circle (white) to create pipe effect
                    hollow_wedge = Wedge((center_x, center_y), TILE_PIXEL_SIZE, 180, 270, 
                    ...
                    full_rect = Rectangle((tile_x, tile_y), TILE_PIXEL_SIZE, TILE_PIXEL_SIZE, 
                                        facecolor='#C0C0C0', edgecolor='#A0A0A0', linewidth=1)
                    patches_list.append(full_rect)
                    hollow_wedge = Wedge((center_x, center_y), TILE_PIXEL_SIZE, 270, 360, 
                                       facecolor='white', edgecolor='#A0A0A0', linewidth=1, zorder=5)
                    patches_list.append(hollow_wedge)
                    ...
                    full_rect = Rectangle((tile_x, tile_y), TILE_PIXEL_SIZE, TILE_PIXEL_SIZE, 
                                        facecolor='#C0C0C0', edgecolor='#A0A0A0', linewidth=1)
                    patches_list.append(full_rect)
                    hollow_wedge = Wedge((center_x, center_y), TILE_PIXEL_SIZE, 0, 90, 
                                       facecolor='white', edgecolor='#A0A0A0', linewidth=1, zorder=5)
                    patches_list.append(hollow_wedge)
                    ...
                    full_rect = Rectangle((tile_x, tile_y), TILE_PIXEL_SIZE, TILE_PIXEL_SIZE, 
                                        facecolor='#C0C0C0', edgecolor='#A0A0A0', linewidth=1)
                    patches_list.append(full_rect)
                    hollow_wedge = Wedge((center_x, center_y), TILE_PIXEL_SIZE, 90, 180, 
                                       facecolor='white', edgecolor='#A0A0A0', linewidth=1, zorder=5)
                    patches_list.append(hollow_wedge)
    elif 18 <= tile_value <= 33:
        # Slope types - create accurate slope shapes using diagonal segments
        if tile_value in TILE_SEGMENT_DIAG_MAP:
            (x1, y1), (x2, y2) = TILE_SEGMENT_DIAG_MAP[tile_value]
            
            # Create accurate slope polygons based on tile definitions
            if tile_value == 18:  # Mild slope up-left, short rise from left edge to middle
                polygon = Polygon([(tile_x, tile_y + TILE_PIXEL_SIZE), 
                                 (tile_x + TILE_PIXEL_SIZE, tile_y + TILE_PIXEL_SIZE),
                                 (tile_x + TILE_PIXEL_SIZE, tile_y),
                                 (tile_x, tile_y + TILE_PIXEL_SIZE/2)], 
                                facecolor='#C0C0C0', edgecolor='#A0A0A0', linewidth=1)
            elif tile_value == 19:  # Mild slope up-right, short rise from middle to right edge
                polygon = Polygon([(tile_x, tile_y + TILE_PIXEL_SIZE), 
                                 (tile_x + TILE_PIXEL_SIZE, tile_y + TILE_PIXEL_SIZE),
                                 (tile_x + TILE_PIXEL_SIZE, tile_y + TILE_PIXEL_SIZE/2),
                                 (tile_x, tile_y)], 
                                facecolor='#C0C0C0', edgecolor='#A0A0A0', linewidth=1)
            elif tile_value == 20:  # Mild slope down-right, short drop from middle to right edge
                polygon = Polygon([(tile_x, tile_y), 
                                 (tile_x + TILE_PIXEL_SIZE, tile_y + TILE_PIXEL_SIZE/2),
                                 (tile_x + TILE_PIXEL_SIZE, tile_y + TILE_PIXEL_SIZE),
                                 (tile_x, tile_y + TILE_PIXEL_SIZE)], 
                                facecolor='#C0C0C0', edgecolor='#A0A0A0', linewidth=1)
            elif tile_value == 21:  # Mild slope down-left, short drop from left edge to middle
                ...
                # Create raised platform shapes
                if tile_value == 22:  # Raised mild slope, platform on left with gentle rise
                    polygon = Polygon([(tile_x, tile_y + TILE_PIXEL_SIZE), 
                                     (tile_x + TILE_PIXEL_SIZE, tile_y + TILE_PIXEL_SIZE),
                                     (tile_x + TILE_PIXEL_SIZE, tile_y + TILE_PIXEL_SIZE/2),
                                     (tile_x, tile_y + TILE_PIXEL_SIZE/4)], 
                                    facecolor='#C0C0C0', edgecolor='#A0A0A0', linewidth=1)
                elif tile_value == 23:  # Raised mild slope, platform on right with gentle rise
                    polygon = Polygon([(tile_x, tile_y + TILE_PIXEL_SIZE), 
                                     (tile_x + TILE_PIXEL_SIZE, tile_y + TILE_PIXEL_SIZE),
                                     (tile_x + TILE_PIXEL_SIZE, tile_y + TILE_PIXEL_SIZE/4),
                                     (tile_x, tile_y + TILE_PIXEL_SIZE/2)], 
                                    facecolor='#C0C0C0', edgecolor='#A0A0A0', linewidth=1)
                elif tile_value == 24:  # Raised mild slope, platform on right with gentle drop
                    polygon = Polygon([(tile_x, tile_y + TILE_PIXEL_SIZE/2), 
                                     (tile_x + TILE_PIXEL_SIZE, tile_y + TILE_PIXEL_SIZE/4),
                                     (tile_x + TILE_PIXEL_SIZE, tile_y + TILE_PIXEL_SIZE),
                                     (tile_x, tile_y + TILE_PIXEL_SIZE)], 
                                    facecolor='#C0C0C0', edgecolor='#A0A0A0', linewidth=1)
                elif tile_value == 25:  # Raised mild slope, platform on left with gentle drop
            ...
            elif tile_value in [26, 27, 28, 29]:  # Short steep slopes
                # Create steep slope shapes
                if tile_value == 26:  # Steep slope up-left
                    polygon = Polygon([(tile_x, tile_y + TILE_PIXEL_SIZE), 
                                     (tile_x + TILE_PIXEL_SIZE/2, tile_y + TILE_PIXEL_SIZE),
                                     (tile_x + TILE_PIXEL_SIZE/2, tile_y),
                                     (tile_x, tile_y + TILE_PIXEL_SIZE/4)], 
                                    facecolor='#C0C0C0', edgecolor='#A0A0A0', linewidth=1)
                elif tile_value == 27:  # Steep slope up-right
                    polygon = Polygon([(tile_x + TILE_PIXEL_SIZE/2, tile_y + TILE_PIXEL_SIZE), 
                                     (tile_x + TILE_PIXEL_SIZE, tile_y + TILE_PIXEL_SIZE),
                                     (tile_x + TILE_PIXEL_SIZE, tile_y + TILE_PIXEL_SIZE/4),
                                     (tile_x + TILE_PIXEL_SIZE/2, tile_y)], 
                                    facecolor='#C0C0C0', edgecolor='#A0A0A0', linewidth=1)
                elif tile_value == 28:  # Steep slope down-right
                    polygon = Polygon([(tile_x + TILE_PIXEL_SIZE/2, tile_y), 
                                     (tile_x + TILE_PIXEL_SIZE, tile_y + TILE_PIXEL_SIZE/4),
                                     (tile_x + TILE_PIXEL_SIZE, tile_y + TILE_PIXEL_SIZE),
                                     (tile_x + TILE_PIXEL_SIZE/2, tile_y + TILE_PIXEL_SIZE)], 
                                    facecolor='#C0C0C0', edgecolor='#A0A0A0', linewidth=1)
                elif tile_value == 29:  # Steep slope down-left
                    polygon = Polygon([(tile_x, tile_y + TILE_PIXEL_SIZE/4), 
                                     (tile_x + TILE_PIXEL_SIZE/2, tile_y),
                                     (tile_x + TILE_PIXEL_SIZE/2, tile_y + TILE_PIXEL_SIZE),
                                     (tile_x, tile_y + TILE_PIXEL_SIZE)], 
...
def create_detailed_entity_representation(entity_type, x, y):
    ...
    Args:
        ...
        x, y: Entity position (ALREADY corrected for padding)
...
def create_final_corrected_visualization():
    """Create final corrected pathfinding visualization with proper tile rendering and positioning."""
    print("=" * 80)
    print("🎯 CREATING FINAL CORRECTED PATHFINDING VISUALIZATION")
    print("=" * 80)
    ...
    try:
        print("\n🎨 Creating final corrected visualization...")
        ...
        # Draw accurate level geometry - TILES AT ORIGINAL POSITIONS (NO PADDING OFFSET)
        print("🏗️ Drawing accurate level geometry...")
        for tile_y in range(level_data.height):
            for tile_x in range(level_data.width):
                ...
                if tile_value != 0:  # Non-empty tile
                    # CORRECTED: Tiles at original positions (no padding offset)
                    pixel_x = tile_x * TILE_PIXEL_SIZE
                    pixel_y = tile_y * TILE_PIXEL_SIZE
                    
                    # Create accurate tile representation using actual tile definitions
                    tile_patches = create_accurate_tile_patches(tile_value, pixel_x, pixel_y)
                    for patch in tile_patches:
                        ax.add_patch(patch)
        
        # Draw entities with corrected positioning - ONLY ENTITIES GET PADDING OFFSET
        ...
        for entity in level_data.entities:
            ...
            # CORRECTED: Only entities get padding offset, not tiles
        ...
        # Draw ninja starting position - ONLY NINJA GETS PADDING OFFSET
        corrected_ninja_x = ninja_coords[0] + (MAP_PADDING * TILE_PIXEL_SIZE)
        corrected_ninja_y = ninja_coords[1] + (MAP_PADDING * TILE_PIXEL_SIZE)
        ...
        # Draw target position - ONLY TARGET GETS PADDING OFFSET
        corrected_target_x = target_x + (MAP_PADDING * TILE_PIXEL_SIZE)
        corrected_target_y = target_y + (MAP_PADDING * TILE_PIXEL_SIZE)
        ...
        # Draw path with movement type color coding - ONLY PATH GETS PADDING OFFSET
        print("➡️ Drawing color-coded path with movement types...")
        if len(path_coords) >= 2 and len(path_edge_types) > 0:
            # Correct path coordinates for padding - ONLY PATH COORDINATES GET OFFSET
            corrected_path_coords = []
            for x, y in path_coords:
                corrected_x = x + (MAP_PADDING * TILE_PIXEL_SIZE)
                corrected_y = y + (MAP_PADDING * TILE_PIXEL_SIZE)
        ...
        ax.set_title(f'Final Corrected Pathfinding Visualization: Ninja → Leftmost Locked Door Switch\n'
                    f'BFS Path: {len(path)} nodes, Accurate Tile Rendering, Correct Positioning',
...
Corrections: Accurate tile rendering, proper entity positioning
Direct Distance: {((corrected_target_x - corrected_ninja_x)**2 + (corrected_target_y - corrected_ninja_y)**2)**0.5:.1f}px"""
        ...
        # Save the final corrected visualization
        output_path = "/workspace/nclone/final_corrected_pathfinding_visualization.png"
        plt.savefig(output_path, dpi=300, bbox_inches='tight', facecolor='white')
        print(f"✅ Saved final corrected pathfinding visualization to: {output_path}")
...
def main():
    """Main function."""
    success = create_final_corrected_visualization()
    
    if success:
        print("\n" + "=" * 80)
        print("🎉 SUCCESS! Final corrected pathfinding visualization created!")
        print("✅ Fixed tile rendering - accurate complex shapes and slopes")
        print("✅ Fixed positioning - tiles at original positions, only entities offset")
        ...
        print("✅ Visualization saved as final_corrected_pathfinding_visualization.png")
        print("=" * 80)
        return 0
    else:
        print("\n" + "=" * 80)
        print("❌ FAILED! Could not create final corrected pathfinding visualization")
        print("=" * 80)
        return 1
